...
Disclaimer:
Worker and Manager initlizes with: 
...
#### Marking explanation
The marking illustrated below is the state at which the work flow starts, it is triggered to move on by a coordinator.
...
for {
		...
		files.ForEach(func(file *object.File) error {
                ...
                TaskSpace.Put(ClientName, TASK, INDEX_FILE, r, n, content ) // p4 -> t5
                ...
			}
			return nil 
		}) // p4 -> t7
...
The actual implementation can be read in files on the github repository: worker/worker.go and manager/initialization.go.

### 3.2 Concurrent Algorithm: tup4fun


For the tup4fun model, all the properties in the tuples have been converted to integer to enable tup4fun at http://www.formalmethods.dk/tup4fun/#tuple-spaces. 
...
The model tries to be as faithfull to the original implementation and test execution, trying to simulate ressources: 2 managers and 5 workers competing to consume tokens.
...
[] // Simulated Manager
   loop true -> IndexSpace.get(?r);
                TaskSpace.put(2, 1, 1, r, 0, 0);
                TaskSpace.get(2, 2, 1, r, 0);

                TaskSpace.put(2, 1, 2, r, 0, 0);
                TaskSpace.get(2, 2, 2, r, 0);

                TaskSpace.put(2, 1, 3, r, 0, 0);
                TaskSpace.get(2, 2, 3, r, 0);

                TaskSpace.put(2, 1, 4, r, 0, 0);
                TaskSpace.get(2, 2, 4, r, 0)
   pool
[] // Simulated Worker
...
To provide a reasonable portion of the transition system, only the simplified tup4fun model's transition system is illustrated. The transition system below shows: The coordinator initating 3 index work flows, a manager that controls the 3 work flow and a worker that receives and execute the 14 tasks. 
